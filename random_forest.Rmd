...
# add_predictor_times adds date as well as other times. This becomes a heavily
# weighted variable which I would like to remove, but I need it for step_holiday
# Does step_holiday modify date and that is why it matters?
...
mod <- rand_forest(mtry = tune(), trees = tune()) %>%
  set_mode("regression") %>%
  set_engine("ranger")
...
metrics %>%  
  filter(.metric == "rmse", parameter == "trees") %>% 
  ggplot(aes(x = value, y = mean)) + 
  geom_point() + 
  labs(title = "RMSE")
```
