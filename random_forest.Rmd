...
Random forests works well with large datasets like the one we are using in our project *mention how many observations there is*. It also has an extremely high accuracy in its predictions and it scales well in general but also compared to decision trees (more on this in the model comparison section)
...
# 
# filtered_data <- filter_by_distance(data, from_station = 'lincoln_memorial', distance_m = 3200)
# 
# hour_data <- get_station_hourly(filtered_data) %>%
#   get_historic_weather() %>%
#   add_sun_is_out() %>%
#   format_weather() %>% 
#   filter(conditions != 'Bad')
...
#lagged_data <- read_csv('lagged_data.csv') %>% 
