...
holidays_date <- left_join(train_data_date, holidays, by= c('year', 'month', 'day'))

 bar <- holidays_date %>%
   filter(year==2020) %>%
   group_by(day, month) %>%
   select(holiday, lincoln_memorial_departures, day, month) %>%
   summarise(lincoln_memorial = sum(lincoln_memorial_departures)) %>%
   na.omit() %>%
