...
data_split <- initial_split(hour_date_date, prop = 0.8)
...
train_date_date%>%
  summarize(mean(lincoln_memorial_departures), max(lincoln_memorial_departures), min(lincoln_memorial_departures), sd(lincoln_memorial_departures)) # how can we get better summary stats? maybe not split by hour?
...
# check how much NA in temprature variable
sum(is.na(train_date_date$temperature)) 
# 9374, we might want to do imputation on this
...
train_date_date %>%
  filter(lincoln_memorial_departures != 0) %>% # notice that we here are dropping 0s because then the visualization will be much clearer
  ggplot(aes(x=lincoln_memorial_departures))+
  geom_density() # this is showing that it is heavily skewed and we might want to transform our dependent variable in our random forest. Aaron said we should look at Box Cox or Eon Johnson transfomation. Or simple log
```
...
train_date_date %>%
  ...
  geom_point(alpha= 0.1, color="blue") + # people cycle more in the middle of the day, like between 1 and 7PM
  geom_smooth()
...
train_date_date %>% 
  ...
  geom_point(alpha=0.1, color="dark red") + # people cycle most in Spring, summer and fall. Suprised that there is no big spike in summer months
  geom_smooth() # why does the smooth line not work here
...
train_date_date %>% # something is off with the year variable, how can we remove the space in the middle?
  ggplot(aes(x = year, y = lincoln_memorial_departures)) +
  geom_point(alpha=0.1) + # overall, more people use the service in 2021 than in 2020
  geom_smooth()
...
train_date_date %>%
  filter(precipitation != 0) %>% #removing 0 to better show relationship
  ggplot(aes(x=precipitation,y=lincoln_memorial_departures))+
  geom_point(alpha=0.1,color='blue')+
  labs(x='Precipitation', y= 'Hourly Departures and Arrivals') +
  geom_smooth() # Negative relationship between bike usage and precipitation as expected
