...
#cbs_data <- read.csv("data/202109-capitalbikeshare-tripdata.csv") # trying with a smaller dataset as cannot load with larger
...
recipe <-
  ...
  step_BoxCox(all_outcomes()) # for decision trees, we use the same recipe as for a random forest model
...
collect_metrics(model_fit) #average rmse across resamples for this model is 2.49 
...
The decision tree estimation produced an RMSE of *INSERT RMSE*.  This is a relatively big RMSE compared to our range of *ideal/expected* values mentioned earlier. 

This could be due to several weaknesses pertaining to decision trees. as they tend to overfit which means that it will be difficult to generalize to novel data. They also tend to produce trees that are overly correlated with each other (as it is more likely to get swayed by points that split the attributes well in both our predictor variables and outcome variable) . 
