...
ridge_data_split <- initial_split(ridge_data, prop = 0.8)

# create the training and testing data
ridge_train <- training(x = ridge_data_split)
ridge_test  <- testing(x = ridge_data_split)

# create 10-fold
folds <- vfold_cv(data = ridge_train, v = 10)
```
...
# create a recipe
ridge_recipe <-
  recipe(formula = departures ~ ., data = ridge_train) %>%
...
bake(prep(ridge_recipe, training = ridge_train), new_data = ridge_test)
```
...
ridge_workflow  <- 
  ...
  add_recipe(recipe = ridge_recipe)
...
# collect metrics
plot <- collect_metrics(ridge_fit_rs, summarize = FALSE)
...
plot %>% 
  ...
  labs(title = "Calculated RMSE Across the 10 Folds - Ridge Regression",
       y = "RMSE_hat") +
  theme_minimal()
```