...
hour_data <- get_station_hourly(filtered_data) %>% 
  ## Make hour_data wider here. This will require an update to setup_for_time_prediction
  get_historic_weather() 

# Eric, could you kindly integrate this to the main code structure?
library(reshape)
data <- data %>% 
  mutate(
...
data1 <- cast(data, hour ~ type + station)
